    ...
    */        
    RGraph.Effects.Pie.Wave = function (obj)
    {
        ...
        */
        var max = RGraph.array_max(obj.data);
        var scale = RGraph.getScale(max);
        obj.Set('chart.ymax', scale[4]);

        original_pie_data = RGraph.array_clone(obj.data);
        __rgraph_pie_wave_object__ = obj;

        // Zero all the bars
        for (var i=0; i<obj.data.length; ++i) {
            obj.data[i] = 0;
            setTimeout('RGraph.Effects.Pie.Wave_inner(' + i + ')', i * 100);
        }
