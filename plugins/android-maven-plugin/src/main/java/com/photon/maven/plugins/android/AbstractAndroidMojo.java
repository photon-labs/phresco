...
public abstract class AbstractAndroidMojo extends AbstractMojo {
	...
	static {
		...
		SUPPORTED_PACKAGING_TYPES.add(AndroidExtension.APKPERF);
	}
	 ...
	 * 
	 * @parameter default-value="${project.basedir}/source/res"
	 */
	...
	/**
	 * The android resources overlay directory. This will be overriden by
	 * resourceOverlayDirectories if present.
	 * 
	...
	/**
	 * The android resources overlay directories. If this is specified, the
	 * {@link #resourceOverlayDirectory} parameter will be ignored.
	 * 
	 * @parameter
	 */
	 ...
	 * 
	 * @parameter default-value="${project.basedir}/source/assets"
	 */
	 ...
	 * 
	 * @parameter default-value="${project.basedir}/source/AndroidManifest.xml"
	 */
	protected File androidManifestFile;

	/**
	 * @parameter expression=
	 *            "${project.build.directory}/generated-sources/extracted-dependencies"
	 * @readonly
	 */
	...
	/**
	 * @parameter expression=
	 *            "${project.build.directory}/generated-sources/extracted-dependencies/res"
	 * @readonly
	 */
	protected File extractedDependenciesRes;
	/**
	 * @parameter expression=
	 *            "${project.build.directory}/generated-sources/extracted-dependencies/assets"
	 * @readonly
	 */
	protected File extractedDependenciesAssets;
	/**
	 * @parameter expression=
	 *            "${project.build.directory}/generated-sources/extracted-dependencies/src/main/java"
	 * @readonly
	 */
	protected File extractedDependenciesJavaSources;
	/**
	 * @parameter expression=
	 *            "${project.build.directory}/generated-sources/extracted-dependencies/src/main/resources"
	 * @readonly
	 */
	protected File extractedDependenciesJavaResources;

	/**
	 * The combined resources directory. This will contain both the resources
	 * found in "res" as well as any resources contained in a apksources
	 * dependency.
	 * 
	 * @parameter expression=
	 *            "${project.build.directory}/generated-sources/combined-resources/res"
	 * @readonly
	 */
	protected File combinedRes;

	/**
	 * The combined assets directory. This will contain both the assets found in
	 * "assets" as well as any assets contained in a apksources dependency.
	 * 
	 * @parameter expression=
	 *            "${project.build.directory}/generated-sources/combined-assets/assets"
	 * @readonly
	 */
	...
	/**
	 * Specifies which device to connect to, by serial number. Special values
	 * "usb" and "emulator" are also valid, for selecting the only USB connected
	 * device or the only running emulator, respectively.
	 * 
	 ...
	 * and apkBuilder will not run. This is probably most useful for a project
	 * used to generate apk sources to be inherited into another application
	 * project.
	 * 
	 ...
	 * 1.6, 2.0, 2.01, 2.1, 2,2 as well as 3, 4, 5, 6, 7, 8. If a platform/api
	 * level is not installed on the machine an error message will be produced.
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * Corresponds to
	 * {@link com.photon.maven.plugins.android.configuration.Sdk#path}.
	 * </p>
	 * 
	 ...
	 * <p>
	 * Corresponds to
	 * {@link com.photon.maven.plugins.android.configuration.Sdk#platform}.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Whether to attach the normal .jar file to the build, so it can be
	 * depended on by for example integration-tests which may then access
	 * {@code R.java} from this project.
	 * </p>
	 * <p>
	 * Only disable it if you know you won't need it for any integration-tests.
	 ...
	 * {@code apk} projects, for including them in their builds.
	 * </p>
	 * <p>
	 * Enabling this setting is only required if this project's source code
	 * and/or res(ources) will be included in other projects, using the Maven
	 * &lt;dependency&gt; tag.
	 * </p>
	 * 
	...
	protected File resolveArtifactToFile(Artifact artifact)
			throws MojoExecutionException {
		Artifact resolvedArtifact = AetherHelper.resolveArtifact(artifact,
	...
	protected void waitForInitialDeviceList(
		...
		if (!androidDebugBridge.hasInitialDeviceList()) {
			...
			if (!androidDebugBridge.hasInitialDeviceList()) {
				getLog().error(
						"Did not receive initial device list from the Android Debug Bridge.");
			}
		}
	}
	 ...
	 */
	protected void deployApk(final File apkFile) throws MojoExecutionException,
	...
	protected void deployDependencies() throws MojoExecutionException,
			MojoFailureException {
		Set<Artifact> directDependentArtifacts = project
		...
		if (directDependentArtifacts != null) {
			for (Artifact artifact : directDependentArtifacts) {
				...
				if (type.equals(APK)) {
					getLog().debug(
							"Detected apk dependency " + artifact
									+ ". Will resolve and deploy to device...");
					final File targetApkFile = resolveArtifactToFile(artifact);
					/*
					 * if (undeployBeforeDeploy) { getLog().debug(
					 * "Attempting undeploy of " + targetApkFile +
					 * " from device..."); undeployApk(targetApkFile); }
					 */
					getLog().debug(
							"Deploying " + targetApkFile + " to device...");
	...
	protected void deployBuiltApk() throws MojoExecutionException,
		...
		File apkFile = new File(project.getBuild().getDirectory(), project
				.getBuild().getFinalName() + "." + APK);
		deployApk(apkFile);
	}

	/**
	 * Determines which {@link IDevice}(s) to use, and performs the callback
	 * action on it/them.
	 * 
		...
		if (androidDebugBridge.isConnected()) {
			...
			if (devices.size() > 0) {
				if (StringUtils.isNotBlank(device)) {
					...
					if ("emulator".equals(device) || ("usb".equals(device))) {
						getLog().info(
								"android.device parameter set to " + device);

						for (IDevice idevice : devices) {
							// use specified device or all emulators or all
							// devices
							if ("emulator".equals(device)
									&& idevice.isEmulator()) {
								getLog().info(
										"Emulator "
												+ DeviceHelper
														.getDescriptiveName(idevice)
												+ " found.");
								deviceFound = true;
								deviceCallback.doWithDevice(idevice);
							} else if ("usb".equals(device)
									&& !idevice.isEmulator()) {
								getLog().info(
										"Device "
												+ DeviceHelper
						...
						getLog().info(
								"android.device parameter set to serial number: " + device);
						if (StringUtils.indexOf(device, ",") > -1) {
							deviceSerialNumberArr = device.trim().split(",");

							for (final IDevice idevice : devices) {
								for (int i = 0; i < deviceSerialNumberArr.length; i++) {
									if (idevice.isEmulator()
											&& (deviceSerialNumberArr[i]
													.equalsIgnoreCase(idevice
															.getAvdName()) || deviceSerialNumberArr[i]
													.equalsIgnoreCase(idevice
															.getSerialNumber()))) {
										getLog().info(
												"Emulator "
					...
					if (!deviceFound) {
						throw new MojoExecutionException(
								"No device found for android.device=" + device);
					}
				} else {
					getLog().info(
	 ...
	 */
	protected String extractPackageNameFromApk(File apkFile)
			throws MojoExecutionException {
		...
		getLog().info(
				getAndroidSdk().getPathForTool("aapt") + " "
						+ commands.toString());
		try {
			executor.executeCommand(getAndroidSdk().getPathForTool("aapt"),
					commands, false);
	...
	protected String extractPackageNameFromAndroidManifest(
			File androidManifestFile) throws MojoExecutionException {
		final URL xmlURL;
		try {
	 ...
	 * @throws MojoExecutionException
	 */
	protected String extractInstrumentationRunnerFromAndroidManifest(
			File androidManifestFile) throws MojoExecutionException {
		final URL xmlURL;
		try {
			xmlURL = androidManifestFile.toURI().toURL();
		...
		try {
			instrumentationRunner = JXPathContext.newContext(documentContainer)
					.getValue("manifest//instrumentation/@android:name",
							String.class);
	...
	protected int deleteFilesFromDirectory(File baseDirectory,
			String... includes) throws MojoExecutionException {
		...
		for (String file : files) {
			final boolean successfullyDeleted = new File(baseDirectory, file)
					.delete();
	 ...
	 * @param includes
	 *            Ant-style include statements, for example
	 *            <code>"** /*.aidl"</code> (but without the space in the
	 *            middle)
	 * @return <code>String[]</code> of the files' paths and names, relative to
	 *         <code>baseDirectory</code>. Empty <code>String[]</code> if
	 *         <code>baseDirectory</code> does not exist.
	 */
	protected String[] findFilesInDirectory(File baseDirectory,
			String... includes) {
	 ...
	 * <p/>
	 * <p>
	 * Current implementation looks for <code>&lt;sdk&gt;&lt;path&gt;</code>
	 * configuration in pom, then System property <code>android.sdk.path</code>,
	 * then environment variable <code>ANDROID_HOME</code>.
	 * <p/>
	 * <p>
